...
char* crypt(char file_name[], struct Keyset key); // Don't forget to free the memory !
char* uncrypt(char file_name[], struct Keyset key); // Don't forget to free the memory !
...
{ // Fonction en cours de création...
    ...
    dest.increment = num_key[0] + num_key[4];
    dest.sinnb = 1;
    free(num_key);
    return dest;
}
...
char* crypt(char file_name[], struct Keyset key)
{
    char *file_content = NULL;
...
char* uncrypt(char file_name[], struct Keyset key)
{
    int x = 0;
    char *file_content = NULL;
    file_content = read_file(file_name);
